...
export default function OrderPage() {
  ...
  const [orderStatus, setOrderStatus] = useState<"idle" | "waiting" | "accepted">("idle");
  const [assignedDriver, setAssignedDriver] = useState<{name:string;car:string;plate:string;phone?:string;eta?:number} | null>(null);
    ...
    <div style={{ position: "relative", height: "100vh", width: "100vw", overflow: "hidden", background: "#0a0a0a" }}>
      ...
      {orderStatus === "waiting" && (
        <div style={{
          ...
          <div style={{ width: 60, height: 60, border: "3px solid #333", borderTop: "3px solid #ef4444", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
          <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#fff", marginTop: 8 }}>
            Searching for driver...
          </div>
          <div style={{ fontSize: 14, color: "#888", textAlign: "center" }}>
            Your order was sent to all available drivers
          </div>
          {priceInfo && (
            <div style={{ marginTop: 12, padding: "10px 24px", background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.3)", borderRadius: 12 }}>
              <span style={{ color: "#4ade80", fontSize: 18, fontWeight: 700 }}>{priceInfo.price} som</span>
            </div>
          )}
        </div>
      )}
      ...
      {/* Driver accepted */}
      {orderStatus === "accepted" && assignedDriver && (
        <div style={{
          position: "fixed", inset: 0, zIndex: 300,
          background: "#0a0a0a",
          display: "flex", flexDirection: "column",
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>Driver is coming!</div>
          
          <div style={{ width: "100%", maxWidth: 340, background: "#111", borderRadius: 20, padding: 20, marginTop: 8, border: "1px solid #222" }}>
            ...
            {priceInfo && (
              <div style={{ marginTop: 12, fontSize: 20, fontWeight: 800, color: "#4ade80" }}>
                {priceInfo.price} som
              </div>
            )}
            {assignedDriver.phone && (
              <a href={`tel:${assignedDriver.phone}`} style={{
                display: "block", marginTop: 16, padding: "14px",
                background: "#166534", borderRadius: 12,
          ...
          <button
            onClick={() => router.push("/")}
            style={{ marginTop: 16, padding: "14px 40px", background: "#ef4444", border: "none", borderRadius: 14, color: "#fff", fontSize: 16, fontWeight: 600 }}
          >
            OK
          </button>
        </div>
